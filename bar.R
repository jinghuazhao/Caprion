...
p + geom_bar(stat = "identity", position = position_dodge(0.9)) + geom_errorbar(limits, position = position_dodge(0.9), width = 0.25) + labs(x = "protein", y = "Effect size", srt=90) + ggtitle("Effect size by protein and platform") + scale_fill_discrete(name = "platform") + theme(axis.text.x = element_text(angle = 45, hjust = 1))
dev.off()
