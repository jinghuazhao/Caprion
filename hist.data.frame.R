    ...
    rugs = FALSE, freq = TRUE, mtitl = FALSE, ...) 
{
    ...
    for (v in x) {
        j <- j + 1
        type <- if (is.character(v) || is.factor(v)) 
            "cat"
        else if (inherits(v, "Date")) 
            "Date"
        else "none"
        lab <- attr(v, "label")
        lab <- if (length(lab) && nchar(lab) > 35) 
            nam[j]
        else label(v, units = TRUE, plot = type != "cat", default = nam[j])
        if (type == "cat") {
            tab <- -sort(-table(v))
            dotchart3(tab, xlab = paste("Frequencies for", lab))
        }
        else {
            type <- if (inherits(v, "Date")) 
                "Date"
            else "none"
            ...
            if (length(unique(w)) >= n.unique) {
                ...
                if (nclass == "default") {
                  ...
                  if (type == "Date") 
                    hist(v, nc, xlab = lab, freq = freq, main = "",...)
                  else hist(v, nclass = nc, xlab = lab, freq = freq, 
                    main = "",...)
                }
                m <- sum(is.na(v))
                pm <- paste("n:", n, " m:", m, sep = "")
                title(sub = pm, adj = 0, cex = 0.5)
                if (na.big && m > 0) 
                  mtext(paste(m, "NAs"), line = -2, cex = 1)
                if (rugs) 
                  scat1d(v, ...)
                if (automf && interactive() && names(dev.list()) != 
                  "postscript" && (i%%prod(mf) == 0)) {
                  if (is.character(mtitl)) 
                    mtitle(mtitl)
                  cat("click left mouse button to proceed\n")
                  locator(1)
                }
                else if (is.character(mtitl) && i%%prod(mf) == 
                  1) 
                  mtitle(mtitl)
            }
        }
