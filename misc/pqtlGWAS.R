...
caprion_prot <- pQTLdata::caprion[["Protein"]]
...
signals <- read.csv(file.path(analysis,"work","caprion_dr.cis.vs.trans"),as.is=TRUE)
caprion_metal <- read.delim(file.path(analysis,"work","tbl_dr.tsv"),as.is=TRUE) %>%
                 dplyr::mutate(hg19_coordinates=paste0(Chromosome,":",Position),Protein=paste0(prot,"_HUMAN")) %>%
                 dplyr::rename(Total=N) %>%
                 dplyr::left_join(pQTLdata::caprion[c("Protein","Gene")]) %>%
                 dplyr::left_join(dplyr::select(signals,prot,SNP,Type),by=c("prot"="prot","MarkerName"="SNP"))
caprion_aggr <- caprion_metal %>%
                dplyr::select(Chromosome,Position,prot,Gene,hg19_coordinates,MarkerName,Allele1,Allele2,Freq1,Effect,StdErr,log.P.,Type) %>%
                dplyr::group_by(Chromosome,Position,MarkerName,hg19_coordinates) %>%
                dplyr::summarise(nprots=dplyr::n(),
...
r2 <- 0.8
build <- 37
metal <- subset(within(caprion_metal,{HLA <- as.numeric(Chromosome==6 & Position >= 25392021 & Position <= 33392022)}),
                select=-c(Chromosome,Position,Direction))
aggr <- subset(within(caprion_aggr,{HLA <- as.numeric(Chromosome==6 & Position >= 25392021 & Position <= 33392022)}),
...
ps_filter <- ps_gcst %>%
             filter(!(snp=="rs601338" & rsid!=snp & grepl("Crohn",trait))) %>%
             filter(!(snp=="rs601338" & grepl("Crohn|Cholelithiasis|Bring up phlegm|High cholesterol|Hypertension",trait) & proxy=="1")) %>%
             filter(!(snp=="rs635634" & grepl("EFO_0001360|EFO_0003827|EFO_0003907",efo) & proxy=="1")) %>%
             filter(!(snp=="rs579459" & grepl("EFO_0008111|EFO_1001395|EFO_0004325|Pulmonary embolism|Deep vein thrombosis",trait) & proxy=="1")) %>%
             filter(!(snp=="rs579459" & grepl("EFO_1001516|EFO_0003827|Pulmonary embolism|Phlebitis and thrombophlebitis",trait) & proxy=="1")) %>%
...
SF <- function(rxc, dn, f="SF-pQTL-IMD-GWAS.png", ch=21, cw=21, h=16, w=17, ylab="Immune-mediated outcomes")
{
  ...
  grid.draw(p)
  grid.text("Protein-pQTL (Nearest gene)", y=-0.02, gp=gpar(fontsize=15))
...
with(rxc_gwas,SF(rxc,dn,f="SF-pQTL-disease-overlap.png",ch=21,cw=21,h=24,w=32,ylab="GWAS diseases"))
