...
The small correlations between the normal deviates between two phases seemed counterintuitve but possibly due to significantly smaller p values at phase II; the impact of the systematic bias in the measurement was unclear. It might be tempting to use more PCs in the model-based clustering with proportion of variance explained increasing from 30.0% to 57.2% from one to 20 PCs; nevertheless much improvement of the classification won't be expected since there is already and perfect separation up to three PCs through R/rgl (dynamic plot not shown here).
