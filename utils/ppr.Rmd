...
An undisclosed pattern was reported but successfully recovered through PCA, as furnished with R's `prcomp`.
...
See a separate document for more details of EPCR-PROT study.
...
For the EPCR-PROC case, both cis and trans pQTL were reported. In pratice, one tends to meta-analyse phase I and II results and in this case, it is possible to work on the piptides (as in the TROMSO study) and/or also only those above limit of detection (_DR, normalised log-intensities for every sample with detection rate >10%).
