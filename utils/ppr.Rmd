...
This visually contrasts two vectors of p values emanating from the same chromosomal positions, and an example on P31946-1433B is given below.
...
It turned to be typical that phase II produced more significant results, Wilcoxon test p=1.

## Overlap of sentinels

Our focus here is on $10^{-5}$, $10^{-6}$ and $5 \times 10^{-8}$.
...
Note that uniprot IDs were used for phase I while protein names were used for phase II, based on all data (_All, normalised log-intensities for every sample).

Directory miamiplot/ contains the Miami plots.

# A case study

See a separate document for full details.

# Reflections

A study of 196 individuals appeared sufficient to see statistical significance at $10^{-5}$ level; however for $5\times 10^{-8}$ even N=1,488 appeared not sufficient though it did boost the number of signals at $10^{-6}$. The fact that little overlaps were seen from $10^{-5}$ and $5\times 10^{-8}$ suggested that the former more likely generated false positives while the latter more likely to be equally unreliable, leaving $10^{-6}$ to be a good balance of false positive and power.

The small correlations between the normal deviates between two phases seemed counterintuitve but to some extent was related to LD; the impact of the systematic bias in the measurement was unclear. It might be tempting to use more PCs in the model-based clustering with proportion of variance explained increasing from 30.0% to 57.2% from one to 20 PCs; nevertheless much improvement of the classification won't be expected since there is already and perfect separation up to three PCs through R/rgl (dynamic plot not shown here).

For the EPCR-PROC case, both cis and trans pQTL were reported.
