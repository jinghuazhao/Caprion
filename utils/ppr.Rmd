...
The documentation considers the reationale and elementary results from the two-phase Caprion proteomics study.
...
$$
\begin{equation*}
invnormal(protein) \sim covariates + genotypes
\end{equation*}
$$
For phase II model, the covariate in includes sample classificaiton according to model-based clustering.
...
To improve the property of statistics involving p values, they are converted to their normal(0, 1) deviates ($z$) for calculation of correlations. Without loss of generality, let $z$>0 (or $z$=$-z$ otherwise) and for a two-sided p value, we have $p=\Phi(-z)+(1-\Phi(z)) = 2\Phi(-z)$ due to symmetry, so $z=\Phi^{-1}(p/2)$ where $\Phi(.)$ is the cumulative distribution function.
